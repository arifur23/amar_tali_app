...
import 'package:amar_tali_app/presentation/Reports/personal_debt_report.dart';
import 'package:flutter/material.dart';
import 'package:provider/provider.dart';

class PersonalDebtScreen extends StatefulWidget {
  const PersonalDebtScreen({Key? key}) : super(key: key);

  @override
  State<PersonalDebtScreen> createState() => _PersonalDebtScreenState();
}

class _PersonalDebtScreenState extends State<PersonalDebtScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    var personalDebtRepository = Provider.of<PersonalDebtRepository>(context, listen: false);
    return Scaffold(
      appBar: AppBar(
        title: const Text('Personal Debt'),
        centerTitle: true,
        backgroundColor: Colors.lightGreen,
        actions: [
          GestureDetector(
            onTap: (){
              Navigator.push(context, MaterialPageRoute(builder: (_) => const PersonalDebtReportScreen()
              ));
            },
            child: Container(
              height: 40,
                                          ...
                                          children: <Widget>[
                                            ...
                                            GestureDetector(
                                              onTap: () async {
                                                ...
                                                await personalDebtRepository.addAPersonalDebt(int.parse(amountController.text), detailsController.text, whomController.text, iSGet, selectedDate.toString()).then((value){
                                                });
                                                setState(() {
                                                  loading = false;
                                                });
